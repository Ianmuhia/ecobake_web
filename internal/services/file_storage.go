...
type FileStorageService interface {
	...
	ProductMultipleFileUpload(filenames []*graphql.Upload, int2 int) ([]string, error)
}
...
func (s *fileStorageService) ProductMultipleFileUpload(filenames []*graphql.Upload, int2 int) ([]string, error) {
	type item struct {
		thumbnail minio.UploadInfo
		err       error
	}
	...
	for _, f := range filenames {
		go func(fh *graphql.Upload) {
			ctx := context.Background()
			ctt := fh.Filename
			var it item
			fname := fmt.Sprintf("%d/%s", int2, fh.Filename)

			it.thumbnail, it.err = s.client.PutObject(ctx, s.bucketName, fname, fh.File, fh.Size, minio.PutObjectOptions{ContentType: ctt})
